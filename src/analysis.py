...
def audit(df: pd.DataFrame) -> Audit:
    ...
    return Audit(
        len(df), len(df.columns), missing, int(df.duplicated().sum()), invalid_hs8, nonpositive,
        int(df["exporter_name"].nunique()) if "exporter_name" in df else 0,
        int(df["ntn"].nunique()) if "ntn" in df else 0,
        int(df["hs8"].nunique()) if "hs8" in df else 0,
        chapters, warnings,
    )
...
def concentration(df: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "exporters_to_25pct": n_to(.25),
        "exporters_to_50pct": n_to(.50),
        "exporters_to_60pct": n_to(.60),
        "exporters_to_80pct": n_to(.80),
        "exporters_to_90pct": n_to(.90),
    }
...
def strategic_screen(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "reported_record_count" in df:
        aggs["reported_records"] = ("reported_record_count", "sum")
    for c in ["email", "telephone"]:
        if c in df:
            aggs[c] = (c, _first_nonnull)
    base = df.groupby(group_keys, dropna=False).agg(**aggs).reset_index()

    # Compute chapter-scale percentile once on the unique company table. This prevents
    # firms with multiple HS8 rows from being over-represented in the percentile base.
    firm = exporter_table(df)[identity + ["rank", "reported_value_rs", "share", "hs8_count", "hs4_count"]].copy()
    firm["firm_scale_percentile"] = firm["reported_value_rs"].rank(pct=True)
    firm = firm.rename(columns={
        "rank": "chapter_rank",
        "reported_value_rs": "firm_chapter_value_rs",
        "share": "firm_chapter_share",
        "hs8_count": "firm_hs8_breadth",
        "hs4_count": "firm_hs4_breadth",
    })
    base = base.merge(firm, on=identity, how="left")

    hs = base.groupby("hs8", dropna=False).agg(
        hs8_total_value_rs=("hs8_value_rs", "sum"),
        firms_in_hs8=("exporter_name", "nunique"),
    ).reset_index()
    ...
    def classify(r):
        ...
        if large_firm and top_product and material_share:
            return "A — high-priority evidence"
        if (top_product and material_share) or (large_firm and scarce):
            return "B — priority review"
    ...
    def reason(r):
        facts = [
            f"chapter rank {int(r.chapter_rank)}",
            f"HS8 rank {int(r.rank_within_hs8)} of {int(r.firms_in_hs8)}",
            f"{r.share_within_hs8:.1%} of observed HS8 value",
        ]
        if int(r.firms_in_hs8) <= 5:
            facts.append(f"only {int(r.firms_in_hs8)} observed firms in HS8")
        if int(r.firm_hs8_breadth) > 1:
            facts.append(f"{int(r.firm_hs8_breadth)} observed HS8 capabilities")
        return "; ".join(facts)
    ...
    return base.sort_values(
        ["_tier", "chapter_rank", "rank_within_hs8", "hs8_value_rs"],
